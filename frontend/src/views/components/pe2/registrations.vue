          ...
          <div style="height: 40px">
            <div class="m-3">
              <vs-button color="success" type="relief" icon="cloud_upload"
                         v-bind:disabled="!validSubmission()"
                         @click="submitRegistration();resetValuesAfterSubmission()">Submit Registration
              </vs-button>
            </div>
          </div>
        </vs-card>
...
export default {
  ...
  methods: {
    ...
    submitRegistration: async function () {
      ...
      await axios.post(`http://localhost:8080/api/v1/registrations`,
